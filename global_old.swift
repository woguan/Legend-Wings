...
class Global {
    ...
    func prioirityLoad(n:Notification.Name){
        
        let atlas = SKTextureAtlas(named: "images-priority")
        atlas.preload {
            ...
            for texture in atlas.textureNames{
                ...
                else if texture.contains("toon_") && texture.contains("_main"){
            ...
            nc.post(name: Notification.Name(rawValue: "PreloadNotification"), object: nil, userInfo: nil)
            // load remaining sprites
            self.normalLoad()
        }
    ...
    private func normalLoad(){
        let atlas = SKTextureAtlas(named: "images")
        
        self.bullet = atlas.textureNamed("toon_1_classic_1")
        for texture in atlas.textureNames{
